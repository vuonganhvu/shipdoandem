<?php
class ControllerExtensionModuleJetimpexBlogArticles extends Controller {
	...
	public function getData() {
		$this->load->language('extension/module/jetimpex_blog_articles');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('jetimpex_blog_articles', $this->request->post);
			} else {
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/jetimpex_blog_articles', 'user_token=' . $this->session->data['user_token'], true),
			'separator' => ' :: '
			);

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/jetimpex_blog_articles', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/jetimpex_blog_articles', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$this->response->setOutput($this->load->view('extension/module/jetimpex_blog_articles', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/jetimpex_blog_articles')) {
